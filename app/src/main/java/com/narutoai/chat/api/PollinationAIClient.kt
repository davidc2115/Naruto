...
class PollinationAIClient {
            ...
            for (attempt in 1..maxRetries) {
                try {
                    ...
                    client.newCall(request).execute().use { response ->
                        when (response.code) {
                            200 -> {
                                // Retourner directement l'URL - Coil devrait gérer
                                android.util.Log.d("PollinationAI", "✅ Image accessible: $imageUrl")
    ...
    private fun buildCharacterPrompt(
        ...
        return buildString {
            ...
            append(", ")
            append("STRICT: match the physical description exactly (age, hair, eyes, body type, height). Do not change these traits. No youthful/teen features, ")
    ...
    private fun buildThumbnailPrompt(
        ...
        return buildString {
            ...
            append(adultSafetySuffix(age))
            append(", ")
            append("STRICT: match described age and physical traits exactly; no youthful/teen features, ")

...
enum class ImageEngine(val displayName: String) {
    POLLINATIONS_FLUX("🌸 Pollinations AI (FLUX Photoréaliste HD - Recommandé)"),
    FREE_SMARTPHONE("✨ Moteur Gratuit (FLUX Photoréaliste HD)"),
...
data class EngineSettings(
    ...
    val geminiImageModelName: String = "gemini-2.5-flash-image",
    ...
    val imageEnginePreference: ImageEngine = ImageEngine.POLLINATIONS_FLUX,
)
...
class SettingsRepository(private val context: Context) {
    ...
    private object Keys {
        ...
        val MOM_STEPMOM_SEEDED = booleanPreferencesKey("mom_stepmom_catalog_seeded_v9")
    }
    ...
    val settings: Flow<EngineSettings> = context.dataStore.data.map { prefs ->
        EngineSettings(
            ...
            geminiImageModelName = prefs[Keys.GEMINI_IMAGE_MODEL_NAME]?.takeUnless { it.isBlank() } ?: "gemini-2.5-flash-image",
